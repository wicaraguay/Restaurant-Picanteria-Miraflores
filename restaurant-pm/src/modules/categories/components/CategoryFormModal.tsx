 ...
 * Incluye soporte para subida de imágenes a Cloudinary y selección de tipo de producto.
 */
...
export const CategoryFormModal: React.FC<CategoryFormModalProps> = ({ isOpen, onClose, onSave, category }) => {
    ...
    const validate = (): boolean => {
        ...
        const productTypeVal = Validators.required(formData.productType, 'Tipo de Producto');
        if (!productTypeVal.valid) newErrors.productType = productTypeVal.error!;
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={isEditing ? 'Editar Categoría' : 'Añadir Categoría'}>
            <form onSubmit={handleSubmit} className="space-y-5 py-2">
                <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-1.5 block ml-1">Nombre de la Categoría</label>
                    <input
                        type="text"
                        name="name"
                        value={formData.name || ''}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-1.5 block ml-1">Tipo de Producto</label>
                        <select
                            name="productType"
                        ...
                        {errors.productType && <p className="text-[10px] text-red-500 font-bold mt-1 ml-1 uppercase">{errors.productType}</p>}
                    </div>
                    <div>
                        <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-1.5 block ml-1">Orden</label>
                        <input
                            type="number"
                            name="sortOrder"
                        ...
                        />
                        <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1 ml-1">Orden de visualización (menor = primero)</p>
                    </div>
                </div>
                ...
                <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-1.5 block ml-1">Imagen de la Categoría</label>
                ...
                <div className="flex items-center p-4 bg-gray-50 dark:bg-dark-900/50 rounded-2xl border border-gray-100 dark:border-dark-800 transition-all hover:border-blue-500/30">
                    ...
                    />
                    <label htmlFor="visibleOnWebsite" className="ml-3 font-black text-gray-700 dark:text-gray-200 text-[10px] uppercase tracking-widest cursor-pointer">Visible en el sitio web</label>
                </div>
                ...
                <div className="flex justify-end pt-6 gap-3">
                    ...
                    <button
                        type="submit"
                        className="px-8 py-3 rounded-2xl bg-blue-600 text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all active:scale-95 flex items-center gap-2"
                        disabled={isUploading}
                    >
                        {isUploading ? (
                            <>
