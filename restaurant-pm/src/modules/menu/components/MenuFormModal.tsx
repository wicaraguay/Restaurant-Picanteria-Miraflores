...
export const MenuFormModal: React.FC<MenuFormModalProps> = ({ isOpen, onClose, onSave, item }) => {
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            setImageFile(file);
            // Mostrar previsualizaciÃ³n local mientras se sube
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.name || !formData.category || formData.price === undefined) return alert('Nombre, CategorÃ­a y Precio son obligatorios.');
            ...
            if (imageFile) {
                try {
                    ...
                    if (error.message === 'CONFIG_MISSING') {
                        alert('Â¡FALTAN LAS CREDENCIALES DE CLOUDINARY!\n\nPor favor, abre el archivo "components/MenuManagement.tsx" y coloca tu Cloud Name y Upload Preset en las lÃ­neas 16 y 17.');
                    } else {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={isEditing ? 'Editar Plato' : 'AÃ±adir Plato'}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <input type="text" name="category" value={formData.category || ''} onChange={handleChange} required placeholder="CategorÃ­a" className={inputClass} />
                ...
                <textarea name="description" value={formData.description || ''} onChange={handleChange} rows={3} placeholder="DescripciÃ³n" className={inputClass}></textarea>
